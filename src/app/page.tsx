      ...
      <main className="mx-auto mt-20 w-full flex flex-col justify-center items-center">
        ...
        <ul className="w-full h-full flex flex-col px-10 mt-10 gap-y-4">
          {items.map((item, index) => (
            <li key={index + 1}>
              <Link
                className="flex items-center justify-between gap-6 p-2 bg-foreground/5 hover:bg-foreground/10 rounded-lg"
                href={item.href}
                target="_blank"
              >
          ...
          {socials.map((social, index) => (
            <li key={index + 1}>
              <Link
                ...
                target="_blank"
                className="text-primary/40 hover:text-primary/65"
              >
